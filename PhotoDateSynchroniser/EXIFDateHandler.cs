...
namespace PhotoDateFixer
{
    ...
    public class EXIFDateHandler
    {
        ...
        // Returns whether or not the current image file has
        // the Date Distributed property item.
        private bool HasDateDigitised(Image image)
        {
            PropertyItem[] propertyItems = GetPropertyItems(image);
            // Ensure that the file has some properties.
            if (propertyItems.Length == 0)
            ...
            foreach (PropertyItem propertyItem in propertyItems)
            {
                // Check if the property item is for the Date Taken or
                // the Date Digitised value.
                if (propertyItem.Id == DateDigitisedCode)
                    return true;
            }
        ...
        public bool SetDateTaken(DateTime date)
        {
            ...
            if (HasDateTaken(image) || HasDateDigitised(image))
            {
                // Assign the Date Taken attribute if it exists
                ...
                // Assign the Date Digitised attribute if it exists
                if (HasDateDigitised(image))
                {
        ...
        public DateTime? GetDateTaken()
        {
            ...
            if (HasDateTaken(image) || HasDateDigitised(image))
            {
                ...
                PropertyItem dateTaken = null;
                if (HasDateTaken(image))
                    dateTaken = image.GetPropertyItem(DateTakenCode);

                PropertyItem dateDigitised = null;
                if (HasDateDigitised(image))
                    dateDigitised = image.GetPropertyItem(DateDigitisedCode);

                // Match to the same value if one doesn't exist
                if (dateTaken == null)
                    dateTaken = dateDigitised;

                if (dateDigitised == null)
                    dateDigitised = dateTaken;
